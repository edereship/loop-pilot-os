...
- **N1. ネットワーク瞬断のリトライ（対話で追加）** — ループ全体を止めうるネットワーク操作のうち未リトライ箇所に、一時的エラー限定の操作リトライ（既定 2 回＝計3試行）を足す。
...
- **D-14** N1: **一時的(transient: ネットワーク/タイムアウト/5xx/接続リセット)エラーのみ、操作ごとに数回リトライ**。決定的(リポ汚染/ブランチ既存/認証/cost_exceeded)はリトライせず即 HALT。設定キー `safety.transient_retry_attempts`（既定 2＝計3試行、0 で無効）。HANDOFF の各操作(push/PR作成/ラベル)に効かせて瞬断を `handoff_failed` になる前に吸収する（既存の in_review `retry(3)` と整合）。使い切って到達した `handoff_failed` の policy は halt（§4）。

### V. 数値の既定
- **D-15** `max_verify_attempts=2` / `max_cost_usd_per_verify=2` / `verify_timeout_minutes=15`（既存 DESIGN REVIEW=2回・self-review=$2・15分に整合）/ `max_recovery_attempts=2` / `transient_retry_attempts=2`。
...
- **FailureReason**: 現在 12 メンバ（`types.ts:15-27`）。`verify_failed` を追加して 13 メンバになる。
...
| -- | -- | -- |
| `claim_failed` / `handoff_failed` / `exception` / `monitor_never_engaged` / `workflow_setup_failed` / `cost_exceeded` | **halt** | インフラ異常・継続は危険。人間へ。瞬断は N1 の操作リトライ（各操作2回＝計3試行）で `handoff_failed` 到達前に吸収済み。ここに到達＝認証/ブランチ保護/権限など本物の問題 |
...
| `looppilot_stopped` | **recover** | Codex recovery turn で復旧（LoopPilot 自動再起動）。尽きたら halt |
...
transient_retry_attempts = 2     # N1: 一時的エラーの操作リトライ回数（計3試行・0 で無効）
