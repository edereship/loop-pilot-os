...
describe("Orchestrator 安全弁 — SIGINT/停止要求フラグ（仕様 §11 / カーネル §7 末尾）", () => {
  it("requestStop() を実装フェーズで立てると、IMPLEMENT 完了後の安全点（SELF-REVIEW 前）で Run=halted(user_interrupt) して停止する", async () => {
    const config = makeConfig({ maxTasksPerRun: 3 });
    const h = makeHarness(config);
    h.source.queue = [issue("issue-A", "TY-1"), issue("issue-B", "TY-2")];
    h.agent.outcomes = [
    ...
    const run = h.store.latestRun()!;
    const sessions = h.store.sessionsForRun(run.id);
    // ES-473: IMPLEMENT 後の安全点で HALT（SELF-REVIEW/HANDOFF は実行されない）
    expect(sessions).toHaveLength(1);
    expect(sessions[0].linearIdentifier).toBe("TY-1");
    expect(sessions[0].state).toBe("implementing");
    // IMPLEMENT のみ実行（SELF-REVIEW は HALT で実行されない）
...
describe("Self-Review (ES-473)", () => {
  ...
  it("requestStop() before self-review -> HALT at safe point", async () => {
    const config = makeConfig({ maxTasksPerRun: 1 });
    const h = makeHarness(config);
    h.source.queue = [issue("issue-A", "TY-1")];
    h.agent.outcomes = [
    ...
    // Request stop after IMPLEMENT completes
    ...
    const run = h.store.latestRun()!;
    expect(run.state).toBe("halted");
    // Self-review should not have run
