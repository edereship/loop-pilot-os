...
describe("状態語彙ユニオン（仕様 §7）", () => {
  ...
  it("FailureReason は仕様 §7 の 14 種の失敗理由を網羅する", () => {
    const all = [
      "agent_no_change",
      "cost_exceeded",
      "exception",
    ...
    expect(all.map(ensureExhaustive).length).toBe(14);
  });
...
describe("ドメイン行型の構造（仕様 §7 データモデル / カーネル §4 スキーマ）", () => {
  ...
  it("TaskSessionRow は nullable 列（worktreePath/prNumber/costUsd/failureReason/stopDetail/agentSummary/monitorStartedAt/endedAt）を許容する", () => {
    const row = {
      ...
      needsHumanLabelAdded: 0,
      designReviewAttempts: 0,
      selfReviewCostUsd: null,
      verifyAttempts: 0,
      recoveryTurnAttempts: 0,
      handoffHeadSha: null,
...
describe("モジュールインターフェース（カーネル §2 / 仕様 §4）", () => {
  ...
  it("TaskSource / AgentRunner / GitPrManager / LoopPilotMonitor / Notifier はインターフェースを満たす実装に代入できる", () => {
    ...
    const git: GitPrManager = {
      ...
      fetchCiLogs: async (_prNumber: number, _branch: string, _headSha?: string): Promise<string | null> => null,
    };
