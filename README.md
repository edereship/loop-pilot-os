...
> **v3 時点のスコープ**: 既存チケット駆動のコアループ + PM 自律化（GROOM フェーズ・横断メモリ）+ 設計品質ゲート（DESIGN REVIEW・セルフレビュー）。QA/バグ自動発見・チケット自動生成・マルチ実行者ルーティング・並列・複数リポ・Linear 以外のタスク源は v4 以降です。
...
```
GROOM ─→ SELECT ─→ CLAIM ─→ DESIGN ─→ DESIGN REVIEW ─→ IMPLEMENT ─→ SELF-REVIEW ─→ HANDOFF ─→ MONITOR ─→ DONE
  │         │         │         │            │               │              │              │          │
  │         │         │         │            │               │              │              │          └─ stopped/closed/CI赤/未起動 → STOPPED → HALT
  │         │         │         │            │               │              │              └─ push/ラベル/遷移 失敗 → STOPPED → HALT
  │         │         │         │            │               │              └─ (fail → IMPLEMENT-fix loop)
  │         │         │         │            │               └─ 無変更/コスト超/例外 → STOPPED → HALT
  │         │         │         │            └─ reject × N超 → STOPPED(design_rejected) → HALT
...
- **DESIGN REVIEW**（v3）: Codex が brief を要求適合・スコープ逸脱・抜け漏れ・リスクの 4 観点でブロッキングレビュー。reject 時は reasons を添えて DESIGN へ戻る（最大 `max_design_review_attempts` 回、既定 2）。超過で `design_rejected` HALT。
- **IMPLEMENT**: worktree 内で `claude -p` をコスト上限付き起動。DESIGN の brief を入力として渡す。**実差分**（`origin/<defaultBranch>..HEAD`）で後条件を判定（自己申告は信用しない）。
- **SELF-REVIEW**（v3）: IMPLEMENT 後に**別セッション**の Claude Code がセルフレビュー。要求/brief/spec との整合性・網羅性を検証し、問題があれば自動修正。Codex レビュー（LoopPilot 経由）と観点を分担し補完関係にする。`self_review.enabled = false` で無効化可能。
...
| `safety.max_design_review_attempts` | DESIGN REVIEW の最大再設計回数（既定 2。超過で `design_rejected` HALT） |
...
| `groom.enabled` | GROOM フェーズの有効/無効（既定 `true`。`false` で v2 相当の動作） |
...
- **永続化**: HALT 時に `docs/memory/` を git commit + push。初回 GROOM 時に空ファイル生成 + DB から impl-results を自動 bootstrap。
...
5. **CLAIM → DESIGN → DESIGN REVIEW → IMPLEMENT → SELF-REVIEW → HANDOFF → MONITOR → DONE** を逐次実行
6. いずれかのセッションが STOPPED したら **HALT**（理由付きで通知してループ終了）
...
| `workflow_setup_failed` | CLAIM 時の worktree/ブランチセットアップが回復不能に失敗 | worktree の残骸を手動掃除し、ブランチ衝突を確認 |
| `design_rejected` | DESIGN REVIEW で `max_design_review_attempts` 回の再設計後も Codex が approve しなかった。`stop_detail` に最後の reject reasons | チケットの曖昧さ・スコープを見直すか、brief を手動で書いて再実行 |
