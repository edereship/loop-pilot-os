...
export function buildScoutReviewPrompt(args: ScoutReviewPromptArgs): string {
  ...
  blocks.push(
    [
      ...
      "",
      "Judge each candidate on exactly these four criteria:",
      ...
      "4. **Evidence type accurate (for `evidence_type: \"objective\"` only)**: Accepted",
      "   `objective` candidates enter an autonomous implementation queue without human review.",
      "   Verify that the evidence block contains reproducible command output — test-runner",
      "   output, assertion failures, or error logs. If the evidence consists solely of",
      "   specification text, commentary, or prose description, reject with an evidence-type",
      "   reason; it must be triaged by a human, not queued for autonomous implementation.",
      "",
